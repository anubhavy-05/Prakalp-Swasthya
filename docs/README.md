# SwasthyaGuide Documentation

This folder contains all project documentation organized by topic.

## 📚 Documentation Index

### Getting Started
- **[Quick Start Guide](QUICK_START.md)** - Get up and running in 5 minutes
- **[Project Summary](PROJECT_SUMMARY.md)** - Overview of SwasthyaGuide features

### Deployment & Setup
- **[PostgreSQL Quick Start](POSTGRESQL_QUICKSTART.md)** - 5-minute database setup
- **[PostgreSQL Migration Guide](POSTGRESQL_MIGRATION.md)** - Complete migration documentation
- **[Deployment Guide](DEPLOYMENT_GUIDE.md)** - Deploy to Render/Railway/Heroku
- **[Deploy Now Guide](DEPLOY_NOW.md)** - Quick deployment checklist
- **[Upgrade Complete](UPGRADE_COMPLETE.md)** - v2.0 upgrade summary

### Technical Documentation
- **[Architecture](ARCHITECTURE.md)** - System design and architecture
- **[PostgreSQL Upgrade Summary](POSTGRES_UPGRADE_SUMMARY.md)** - Technical changes for v2.0
- **[Multilingual Support](MULTILINGUAL.md)** - Language detection and responses

### Feature Documentation
- **[Advanced Image Features](ADVANCED_IMAGE_FEATURES.md)** - Image analysis capabilities

---

## 🏗️ Architecture Overview

SwasthyaGuide is a multilingual healthcare chatbot with:
- **8 Indian languages** support
- **WhatsApp integration** via Twilio
- **PostgreSQL database** for scalability
- **Image analysis** for skin conditions
- **Emergency detection** with instant alerts
- **Clinic finder** for 8+ Indian cities

## 🔗 Quick Links

- [Main README](../README.md)
- [Source Code](../src/)
- [Database Models](../database/)
- [Scripts](../scripts/)
- [Tests](../tests/)

---

**Version:** 2.0.0  
**Last Updated:** February 2026
